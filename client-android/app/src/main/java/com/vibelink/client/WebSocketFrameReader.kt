...
class WebSocketFrameReader private constructor(
    ...
    companion object {
        fun connect(baseUrl: String, token: String, displayId: String?): WebSocketFrameReader {
            ...
            val path = "/stream-ws?token=$encodedToken$displayQuery"
            val socket = Socket(host, port).apply { soTimeout = 15000 }
            ...
            if (!response.startsWith("HTTP/1.1 101")) {
                socket.close()
                throw IllegalStateException("websocket handshake failed: ${response.lineSequence().firstOrNull().orEmpty()}")
            }
            return WebSocketFrameReader(socket, input)
        }
...
private fun BufferedInputStream.readRequired(): Int {
    val value = read()
    if (value == -1) throw IllegalStateException("websocket stream ended")
    return value
}
...
private fun BufferedInputStream.readFully(target: ByteArray) {
    ...
    while (offset < target.size) {
        ...
        if (read == -1) throw IllegalStateException("websocket stream ended")
        offset += read
    }
}
