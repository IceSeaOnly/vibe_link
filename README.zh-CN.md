...
当前项目是可端到端运行的 MVP。整体实现保持轻量：macOS 端是 Swift 命令行服务，Android 端是非常精简的原生客户端，屏幕流使用 MJPEG over HTTP，控制通道使用带共享 token 鉴权的 JSON API。当前可下载 Android APK 仅 **878 KB**，手机端更接近一个聚焦的远程开发遥控器，而不是臃肿的远程桌面套件。Android 客户端和 Web 管理端均支持中英文界面。
...
- **Mac 屏幕实时画面**：macOS 服务端通过 `screencapture` 采集显示器画面，并以 MJPEG 流输出。
- **显示器选择**：服务端暴露当前 macOS 显示器列表，Android 客户端可以切换不同显示器画面。
...
- **超轻量 Android 客户端**：当前可下载 APK 仅 **878 KB**，安装和更新都很轻。
...
传统远程桌面能显示画面，但通常把手机当成一个很小的桌面显示器。终端工具很高效，但看不到浏览器弹窗、IDE 状态、桌面权限提示或页面预览。VibeLink 正是为本地 Mac 开发流程中的中间地带而设计：既能看到屏幕，又能精准交互；既能发送文本和快捷动作，又保留轻量协议，后续可以从当前 MJPEG MVP 演进到更低延迟的传输方案。

## 截图

<p align="center">
  <img src="screenshots/zh/app.png" alt="鹊桥 Android 应用" width="280">
</p>

| 控制按钮 | 快捷回复 |
| --- | --- |
| ![控制按钮](screenshots/zh/Control_buttons.png) | ![快捷回复](screenshots/zh/Quick_replies.png) |

| 命令 | 快捷按钮 |
| --- | --- |
| ![命令](screenshots/zh/Commands.png) | ![快捷按钮](screenshots/zh/Shortcuts.png) |

## 架构

```mermaid
flowchart LR
    Android["Android 客户端<br/>原生 Kotlin UI"] -->|"GET /health<br/>GET /stream?token=..."| Server["macOS 服务端<br/>SwiftPM CLI"]
    Android -->|"POST /api/control<br/>POST /api/commands/run"| Server
    Admin["Web 管理端<br/>端口 + 1"] -->|"GET/POST /admin/api/config"| Server
    Server --> Capture["屏幕采集<br/>screencapture"]
...
- 默认应用端口：`8765`
- 默认管理端口：`8766`
- 屏幕流：`multipart/x-mixed-replace` MJPEG
- 控制通道：HTTP JSON API
- 输入模拟：macOS `CGEvent` 与剪贴板
- 屏幕采集：`/usr/sbin/screencapture`
...
- 画面渲染：解析 multipart MJPEG 并解码为 `Bitmap`
...
在 Android 上打开 VibeLink，输入 Mac 局域网地址和 token，然后点击 **Connect**。
...
| `GET` | `/stream?token=<token>` | MJPEG 屏幕流 |
...
- 将 MJPEG 替换为 WebRTC 或其他更低延迟传输方案。
- 加入基于设备密钥的配对流程和二维码引导。
